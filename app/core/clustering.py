...
#
# 0.80 → 0.82 (2026-08-18). 같은 벡터·같은 정답으로 0.72~0.92를 훑어 재고
# 옮겼다(cluster_report 기준):
#
#     0.80   정밀도 0.959 · 재현율 0.949 · F1 0.954
#     0.82   정밀도 1.000 · 재현율 0.971 · F1 0.985
#
# **두 지표가 함께 올랐다** — 정밀도를 사려고 재현율을 판 것이 아니다.
# 0.80에서 '수질통계'와 '월간실적보고'가 한 묶음이 되던 24짝이 사라졌고,
# '수질통계'가 둘로 갈리던 13짝도 사라졌다. 남은 오류는 문서 한 건이
# 미분류로 남는 것뿐인데, 그것은 확신 없는 문서를 업무에 밀어 넣지 않는다는
# 이 제품의 약속대로다(§7).
#
# 0.81·0.82·0.83이 모두 같은 결과를 내는 고른 구간이라 그 한가운데를 골랐다
# — 한 점에서만 좋은 값은 다음 자료에서 무너진다. 0.84부터는 미분류가
# 2건에서 6건으로 뛰므로 위쪽 가장자리는 가파르다.
#
# 여전히 **합성 표본** 기준이다. 실제 인수인계 자료 파일럿 뒤에 다시 잰다.
